...
$$
\operatorname{logit}(\mu_{ijk}) = \beta_{0jk} + \beta_{1jk} \operatorname{session}_{ijk} + \beta_{2jk}  \operatorname{ID}_{ijk} + \beta_{3jk}\operatorname{session\_intervention}_{ijk} 
$$ {#eq-one}


In @eq-one $\beta_{0jk}$ represents the baseline log-odds at the first baseline session. $\beta_{1jk} \operatorname{session}_{ijk}$ is the baseline slope on log-odds scale, i.e., the change in outcome during the baseline phase at the following sessions. $\beta_{2jk} \operatorname{ID}_{ijk}$ represents the log-odds level change, i.e., the difference between the predicted outcome at the first baseline session and the predicted outcome when the intervention is introduced (first intervention session). Finally, the change in log-odds slope predicted by the interaction between the session and phase is estimated by $\beta_{3jk}\operatorname{session\_intervention}_{ijk}$, with the intervention trend presented by $\beta_{1jk}$ + $\beta_{3jk}$. 

We will introduce a session-by-intervention parameter centered at the intervention introduction within each case. Specifically, the first intervention session will be coded as 0, baseline sessions will be coded as 0 on this variable, and the following intervention sessions will increase sequentially. This indexing will allow us to interpret the level-shift coefficient (i.e., IDIntervention coefficient) as the difference between the predicted baseline outcome and the predicted intervention outcome at the first intervention session. Furthermore, the session_intervention coefficient will represent the change in slope between the baseline and intervention phases.
...
Breaking this down, we set the initial equation at Level 1 (sessions within participants) from @eq-one, and then introduce the group-centered IQ predictor at the participant level nested within studies (Level 2):

$$
\begin{align}
\beta_{0jk} = \alpha_{00k} + \eta_{01} IQ^c_{jk} + \psi_{0jk} \\
\beta_{1jk} = \alpha_{10k} + \eta_{11} IQ^c_{jk} + \psi_{1jk} \\
\beta_{2jk} = \alpha_{20k} + \eta_{21} IQ^c_{jk} + \psi_{2jk} \\
\beta_{3jk} = \alpha_{30k} + \eta_{31} IQ^c_{jk} + \psi_{3jk} \\
\end{align}
$$ {#eq-iql2}

@eq-iql2 describes the centered IQ introduced as a participant-level fixed effect moderator of baseline ($\eta_{01}\operatorname{IQ^c}_{jk}$), the session (baseline trend), phase (level shift), and the session-by-phase (trend) ($\eta_{11}\operatorname{IQ^c}_{jk}$, $\eta_{21}\operatorname{IQ^c}_{jk}$, $\eta_{31}\operatorname{IQ^c}_{jk}$, resp.). $\psi_{0jk}$, $\psi_{1jk}$, $\psi_{2jk}$, $\psi_{3jk}$ represent the individual-level random effects following the same structure as presented in @eq-three.
...
We will graphically inspect the data to estimate the growth curve based on sessions, to determine whether time influences the outcome linearly (@eq-one) or non-linearly (@eq-thirteen). @eq-thirteen describes a generalized additive model (GAM) with session assumed to be a non-linear predictor. 
...
Accordingly, we will specify the generalized additive model as described in @eq-thirteen, where $\beta_{0jk}$ represents the baseline performance, $\beta_{1jk}\operatorname{ID}_{ijk}$ represents the level change between phases, but unlike @eq-one, baseline slope and difference between baseline and intervention slope coefficients are modeled as smooth terms $f_0(session_{ijk})$ and $f_1(session\_intervention_{ijk})$. Multilevel structure of study- and participant-level random effects is described in @eq-four and @eq-two. \

$$
\operatorname{logit}(\mu_{ijk}) = \beta_{0jk} + \beta_{1jk}\operatorname{ID}_{ijk} + f_0(session_{ijk}) + f_1(session\_intervention_{ijk})
$$ {#eq-thirteen}

The smoothing terms $f_0(session_{ijk})$ and $f_1(session_intervention_{ijk})$ consist of a sum of $M$ simpler basis function $\operatorname{b_{m}}(\operatorname{session}_{ijk})$ and its corresponding coefficients $\beta_{m}$, as described in @eq-spline (@Pedersen2019HGAM). We will use the default values from the smooths function in $\texttt{brms}$, provided by the $\texttt{mgcv}$ package (@mgcv).

$$
f_0(session_{ijk}) = \sum_{m=1}^{M}\beta^{(0)}_{m}\operatorname{b_m}(session_{ijk})
f_1(session\_intervention_{ijk}) = \sum_{m=1}^{M}\beta^{(1)}_{m}\operatorname{b_m}(session\_intervention_{ijk})
$$ {#eq-spline}
...
data <- read_csv(here("analysis", "mydata.csv")) %>% 
  ...
  # Keep only case/outcome series that contain both phases
  group_by(study, case, measure) %>% 
  filter(any(ID == "Intervention")) %>% 
  ungroup() %>% 
  mutate(
    ID = factor(ID, levels = c("Baseline", "Intervention")),
    intervention = if_else(ID == "Intervention", 1L, 0L)
  ) %>% 
  group_by(study, case, measure) %>% 
  arrange(x, .by_group = TRUE) %>% 
  # ITS https://chrissyhroberts.github.io/Code_Book/Interrupted_Time_Series.html#part-one---uncontrolled-its-one-intervention
  mutate(
    # Session starts at 0 for each case/outcome
    session = row_number() - 1L,
    # First intervention session within this case/outcome
    first_intervention_session = min(session[intervention == 1L], na.rm = TRUE),
    # 0 throughout baseline and 0 at the first intervention point
    session_intervention = if_else(
      intervention == 1L,
      session - first_intervention_session,
      0L
    )
  ) %>% 
  ungroup() %>% 
  mutate(
    ID = relevel(ID, ref = "Baseline"),
    case_id = interaction(study, case, measure, drop = TRUE)
  )
...
data_demo <- if (run_fast) {
  data %>%
    group_by(study, case, measure, ID) %>%
...
model_formula <- bf(success_rate|trials(success_rate + fail_rate) ~ session + ID + session_intervention + (1 + session + ID + session_intervention | study/case)) 

# nonlinear session formula - run in the real analysis
#model_formula2 <- bf(success_rate|trials(success_rate + fail_rate) ~ + ID + s(session) + s(session_intervention) + (1 + session + ID + session_intervention | study/case)) 

model_formula2 <- bf(success_rate | trials(success_rate + fail_rate) ~ ID + s(session, k = 4) + s(session_intervention, k = 4) + (1 + session + ID + session_intervention | study/case)) #test formula 
...
formula_iq <- bf(
 success_rate|trials(success_rate + fail_rate) ~
    (session + ID + session_intervention) * IQ_gpc +
    (1 + session + ID + session_intervention | study/case)
)
...
extract_model_terms <- function(fit, model_label) {
  s  <- summary(fit)
  vc <- VarCorr(fit)
  ...
  fixed_tab <- as.data.frame(s$fixed) |>
    ...
    dplyr::select(
      Model, Group, Parameter, Estimate, Est.Error,
      `l-95% CI`, `u-95% CI`, Rhat, Bulk_ESS, Tail_ESS
    )

  get_sd_tab <- function(group_name) {
    if (is.null(vc[[group_name]]$sd)) return(NULL)

    as.data.frame(vc[[group_name]]$sd) |>
...
out <- dplyr::bind_rows(
  extract_model_terms(fit1, "Fit 1"),
  extract_model_terms(fit2, "Fit 2"),
  extract_model_terms(fit3, "Fit 3"),
  extract_model_terms(fit4, "Fit 4")
)
...
newdata_level <- tidyr::crossing(
  ...
  session = data_demo$session,
  session_intervention = 0,
...
rq_one_pred_study <- avg_predictions(fit1,
                               newdata = newdata_level,
                               by = c("ID", "study"),
                               re_formula = ~(1 + session + ID + session_intervention | study), #random effects of study
                               type = "response",
                               conf_level = 0.9) 
...
pred_grid_iq <- tidyr::crossing(
  ...
  session = data_demo$session,              
  session_intervention = 0, # hold session constant at 0
...
IQ_rq_one_pred_study <- avg_predictions(
  ...
  re_formula = ~ (1 + session + ID + session_intervention | study/case), 
  type = "response",
  conf_level = 0.9
)
...
fit1 %>% 
  spread_draws(`b_session_intervention`, r_study[study, `session_intervention`]) %>%
  # add the grand mean to the group-specific deviations
  mutate(mu = `b_session_intervention` + r_study) %>%
  ungroup() %>%
  mutate(study = str_replace_all(study, "[_]", " ")) %>% 
  # plot
  ggplot(aes(x = mu, y = reorder(study, mu))) +
...
fit1 %>% 
  spread_draws(`b_session_intervention`, `r_study:case`[`study:case`, `session_intervention`]) %>%
  # add the grand mean to the group-specific deviations
  mutate(mu = `b_session_intervention` + `r_study:case`) %>%
  ungroup() %>%
  mutate(`study:case` = str_replace_all(`study:case`, "[_]", " ")) %>% 
  # plot
  ggplot(aes(x = mu, y = reorder(`study:case`, mu))) +
